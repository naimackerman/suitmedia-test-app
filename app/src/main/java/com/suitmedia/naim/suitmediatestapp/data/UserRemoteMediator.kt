...
import com.suitmedia.naim.suitmediatestapp.database.RemoteKeys
...
class UserRemoteMediator(
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, UserResponseItem>) =
        state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, UserResponseItem>) =
        state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, UserResponseItem>) =
